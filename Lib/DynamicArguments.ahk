Class ot {
    ...
    AssembleFormatString() {
        ...
        if this.HasKey("dateformat") {
            for Parameter, Value in this.Arguments {
                if Value.control="datetime" {
                    FormatTime timestamp,% Value.Value, dd.MM.yyyy
            ...
            for Parameter, Value in this.Arguments {
                ...
                if (Tab=Value.Tab3Parent) {
                    ...
                    if (Value.Control="Edit") {
                        ...
                        gui ParamsGUI: add, DateTime, % Value.ctrlOptions " h30 vv" Parameter, % "dd.MM.yyyy"
                        guicontrol % "ParamsGUI:",v%Parameter%,% DateParse(script_FormatEx(Value.Value, AHKVARIABLES))
                    } else {
                        if Value.HasKey("Link") {
